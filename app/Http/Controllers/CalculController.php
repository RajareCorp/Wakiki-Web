...
class CalculController extends Controller
{
    ...
    for($i = 0; $i < $total; $i++) {
        ...
        $newTimeCheck = str_replace(',', '.', date('Y-m-d').substr($ligne, 0, 12));
        $lastTime = DateTime::createFromFormat('Y-m-d H:i:s.u', $lastTimeCheck);
        $newTime = DateTime::createFromFormat('Y-m-d H:i:s.u', $newTimeCheck);
        ...
        if(strpos($ligne, "[Information (jeu)]") !== false){
            ...
            if($lastTime < $newTime){
                ...
                $optiFile = fopen(public_path().'\time.txt', 'w');
                fwrite($optiFile, $newTimeCheck);
                fclose($optiFile);
            }
        }
    }
